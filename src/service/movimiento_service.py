...
def get_all():
    ...
    FROM movimientos mv
    JOIN lotes l 
        ON mv.id_lote = l.id_lote
    JOIN usuarios u 
        ON mv.id_usuario = u.id_usuario
    LEFT JOIN ubicaciones ori 
        ON mv.id_ubicacion_origen = ori.id_ubicacion
    LEFT JOIN ubicaciones des 
        ON mv.id_ubicacion_destino = des.id_ubicacion
